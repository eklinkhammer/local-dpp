...
createWorld :: Double -> Int -> Int -> IO World
createWorld sideLength agentCount maxAgents = do
  agents <- randomList agentCount (randomAgent sideLength)
  obstacles <- randomList (agentCount `div` 2) (randomObstacle sideLength maxAgents 10)
...
startExperiment n k gens worldSize steps maxAgents epsilon fitFunc = do
  startingWorld  <- createWorld worldSize n maxAgents
...
runExperiment gens epsilon steps sizePool world fit population = do
  ...
  nextGen        <- runExperiment (gens - 1) epsilon steps sizePool newW fit population
  return nextGen
...
randomObstacle locBound maxAgents maxValue = do
  ...
  reward <- randomRIO (0.5, (locBound / 5))
